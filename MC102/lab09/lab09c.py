#João Gabriel Segato Kruse RA:218616
    ...
    for j in range(len(lista[i])):
        for k in range(len(lista[i][j])-1):
            if lista[i][j][k] + 1 != lista[i][j][k+1]:
                delete.append(j)
                break
possibilidades = [x for i,x in enumerate(possibilidades) if i not in delete ]
nums = [x for i,x in enumerate(nums) if i not in delete]
lucros, lc = [], 0
for i in range(len(possibilidades)):
